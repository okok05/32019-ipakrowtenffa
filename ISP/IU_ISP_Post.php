<?php
   ...
   if(isset($_POST['idISP']))
   {
	  ...
	  $emails = $_POST['txtEmails'];

	  
	   $requete = $bdd->prepare('delete from seedadmin where id_isp = ?');
       $requete->execute(array($idISP));
	  
	  $requete = $bdd->prepare('insert into seedadmin Values(?,?,?)');
      $requete->execute(array(NULL,$idISP,$emails));
   ...
   else
   {
	  ...
	  $isp    =  $bdd->lastInsertId();
	  $emails = $_POST['txtEmails'];

	  $requete = $bdd->prepare('insert into seedadmin Values(?,?,?)');
      $requete->execute(array(NULL,$isp,$emails));
