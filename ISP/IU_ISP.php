<?php
	...
	if(isset($_GET['id_ISP']))
	{
		...
		$requete = $bdd->prepare('select email from seedadmin where id_isp = ?');
	    $requete->execute(array($id_ISP));
		extract($requete->fetch());
	}
				...
				<div class="content">
					...
					<form class="form-horizontal" method="POST" action="IU_ISP_Post.php" enctype="multipart/form-data">
					    ...
					    <fieldset class="content-group">
									...
									<div class="form-group">
										<label class="control-label col-lg-2">Emails Test</label>
										<div class="col-lg-10">
											<input type="text" name="txtEmails" id="txtEmails" class="form-control" value="<?php echo $email;?>">
										</div>
									</div>
