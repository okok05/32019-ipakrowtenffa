												...
												<div class="col-md-4">
													...
													<select name="cmbDomain[]" class="select-clear" data-placeholder="Select Domain">
													  <?php
														...
														foreach($DOMAINS as $dom){?>
															<option value="<?php echo $dom?>"><?php echo $dom?></option><?php
														}
													  ?>
													</select>
												</div>
...
<div id="modal_theme_danger" class="modal fade">
	...
	<script>
	 ...
	 $('#btnAddRow').click(function(){
		...
		$('#cont').append('<div class="form-group ipRow"><div class="col-md-4"><textarea name="txtIP[]" class="form-control" style="resize:none;overflow-y: scroll;"></textarea></div><div class="col-md-4 cmbDomain"> <?php $requete = $bdd->query('select * from domain'); echo '<select name="cmbDomain[]" class="form-control">';$domains=array(); while($row = $requete->fetch()) {$domains[]=$row["name_Domain"];}$DOMAINS=array();foreach($domains as $dom){$d=explode(".",$dom);if(count($d)>2){$dom=array_slice($d,-2,2);$domain=$dom[0].".".$dom[1];}else{$domain=$d[0].".".$d[1];}if(!in_array($domain,$DOMAINS)){	$DOMAINS[]=$domain;	}		}		foreach($DOMAINS as $dom){?>		<option value="<?php echo $dom?>"><?php echo $dom?></option><?php	}echo '</select>';?></div><a class="btn border-warning-400 text-warning-400 btn-flat btn-rounded btn-icon btn-xs valign-text-bottom removeRow" title="Delete ROW"><i class="icon-minus3"></i></a></div>'); 
