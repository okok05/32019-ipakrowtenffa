...
if(isset($_POST['id_Offer']))
{
	...
	$API_KEY 			=	"6f1cfe8ef9a989bd58cca404617b849ea64fa25b642cbce04207d80bbc03850f";
	...
	if (  (is_numeric($id_offer))    and ($id_offer > 0) )
	{
		...
		)
		{
			...
			//$sponsorSupressionFileURL =       "1___https://mailer.optizmo.net/sm-zqff-h62-c1d5ed0d6e12a24df07c6c033920d927&one=1&icma=6193a238bbb06c1e7b8035aa886ee417";
								...
								if($optismoAccessToken)
								{
									if($sponsorSupressionFileURL)
									{
										...
										if($campaignAccessKey)
										{
											...
											//echo "<br/>Suppression File Link :".$suppressionFileLink."<br/>";
...
{
	return "/var/www/exactarget/Offer/Suppression/TempExtraction/";
}
...
function 	renameSupressionFile($p_folder,$p_id_offer,$p_sid_offer,$p_id_sponsor)
{
	...
	{
		$supp_fine_name	=	'/var/www/exactarget/Offer/Suppression/sp'.$p_id_sponsor.'-'.$p_id_offer.'-'.$p_sid_offer.'-MD5.txt';
...
function	moveSupressionFile($p_id_offer,$p_sid_offer,$p_id_sponsor)
{
	...
	$source_folder		=	'/var/www/exactarget/Offer/Suppression/TempExtraction/'.$source_file;
	$destination_folder	=	'/var/www/exactarget/Offer/Suppression/'.$source_file;
...
{
	$source	=	'/var/www/exactarget/Offer/Suppression/'.$source_file;
	$target	=	'/var/www/exactarget/Offer/Suppression/temp_'.$source_file;
