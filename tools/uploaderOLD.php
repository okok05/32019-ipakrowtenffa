						...
						<div class="col-lg-6">
							<div class="panel panel-flat">
								<div class="panel-heading">
									<h2 class="panel-title"><u>Root Document :</u>  /var/www/exactarget/</h2>
				                ...
				                <div class="panel-body">
									...
									<form class="form-horizontal" method="POST" id="frmUploadExactarget" name="frmUploadExactarget">
											...
											<div class="col-lg-12">
												<div class="form-group">
													...
													<div class="col-lg-10">
														<div class="row">
															<div class="col-md-12">
																<input type="text text-bold" class="form-control" name="txtFilePath" id="txtFilePath" placeholder="/var/www/exactarget/"  />
															</div>
														</div>
													</div>
												</div>
										...
										<center>
								<?php
									if(isset($_POST['btnUpload']) and isset($_POST['txtFilePath']) and !empty($_POST['txtFilePath']) and isset($_POST['cbxServers']) )
									{
										//var_dump($_POST);
										$path_file_to_upload	=	$_POST['txtFilePath'];
										$source_server			=	$_POST['cbxServers'];
										if( (!empty($path_file_to_upload)) and ($source_server>0) and (is_numeric($source_server))   )
										{
											...
											if($is_recursive_upload)
											{
												...
												if($server)
												{
													...
													$folder		=	'/var/www/exactarget/'.$folder;
														...
														foreach($files as $file)
														{
															...
															else
															{
																...
																if(in_array(pathinfo($file, PATHINFO_EXTENSION), $permitted_extensions)) 
																{
																	...
																	download_file_from_remote_server($source_server,get_full_path($file,$path_file_to_upload),'/var/www/exactarget/tools/DIRECTORY_UPLOAD/');
												
																	//Uploader le fichier dans tous les serveurs :
																	upload(get_full_path($file,$path_file_to_upload));
																}
															}
														}
													}
											...
											{
												//Download the file from source server to the master server :
												download_file_from_remote_server($source_server,$path_file_to_upload,'/var/www/exactarget/tools/DIRECTORY_UPLOAD/');
										
												//Uploader le fichier dans tous les serveurs :
												upload($path_file_to_upload);
											}
										}
									}
								?>
...
<?php 
	...
	function upload($p_path_file_to_upload)
	{
		echo "<br/><br/><br/>";
			...
			if(file_exists('/var/www/exactarget/tools/DIRECTORY_UPLOAD/'.$filename))
			{
				...
				while($server   =   $cmdGetServers->fetch())
				{
					...
					if($connection) 
					{
						...
						{
							ssh2_scp_send($connection, '/var/www/exactarget/tools/DIRECTORY_UPLOAD/'.$filename, '/var/www/exactarget/'.$p_path_file_to_upload, 0777);
	...
	function download_file_from_remote_server($p_id_server,$p_remote_file,$p_local_file)
	{
		if( ($p_id_server>0) and (!empty($p_remote_file))  and (!empty($p_local_file)) )
		{
			include('../Includes/bdd.php');
			$sqlGetServers  =   
			'
			...
			if($server)
			{
				...
				if(function_exists("ssh2_connect")) 
				{
					...
					if($connection) 
					{
						...
						if(ssh2_auth_password($connection, $login, $password))
						{
							ssh2_scp_recv($connection,'/var/www/exactarget/'.$p_remote_file,$p_local_file.$filename);
